...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (
        ...
        f"🔔 *Abone Durumu:* {'✅ Aktif' if is_subscribed else '❌ Pasif'}"
    )
    ...
    while True:
        try:
            ...
            if current_time - last_scrape_time >= config.CHECK_INTERVAL:
                print(f"🔍 Starting grant scrape cycle at {datetime.now(TURKEY_TZ).strftime('%H:%M:%S')}")
                ...
                if current_grants:
                    ...
                    if new_grants:
                        ...
                        if subscribed_users:
                            # Format notification
                            message = "🚨 *FIRST SİTESİNDE YENİ HİBE BİLDİRİMİ!* 🚨\n\n"
                            for i, grant in enumerate(new_grants[:5], 1):
                                grant_text = grant[:80] + "..." if len(grant) > 80 else grant
                                message += f"{i}. *{grant_text}*\n"
                            
                            if len(new_grants) > 5:
                                message += f"\n... ve {len(new_grants) - 5} tane daha\n"
                            
                            message += f"\n🔗 [Detaylı İncelemek İçin Tıklayın]({config.GRANT_URL})"
                            ...
                            for chat_id in subscribed_users:
                                try:
                                    ...
                                    for grant_text in new_grants:
                                        grant = DB.add_grant(grant_text)
                                        DB.add_notification(chat_id, grant.id)
...
async def main():
    ...
    print(f"⏰ Scrape interval: {config.CHECK_INTERVAL} seconds ({config.CHECK_INTERVAL // 60} minutes)")
    ...
    async with app:
        ...
        # IMPORTANT: start_polling() is non-blocking, it returns immediately.
        # Without this wait, the script would fall straight through to the
        # finally block and shut the bot down right after starting it.
        # This keeps the bot alive until it receives a stop signal (Ctrl+C
        # locally, or SIGTERM from the host on Render).
