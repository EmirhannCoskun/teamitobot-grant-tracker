...
def test_status_command(fake_update, fake_context):
    ...
    with patch(
        ...
        return_value={"users": 10, "scrapes": 5, "started": None, "last_scrape": None},
...
def test_notification_batching_5(fake_application):
    ...
    with patch(
        "bot.DB.get_pending_notifications", return_value=fake_notifications
    ), patch("bot.DB.mark_notification_sent", side_effect=fake_mark_sent), patch(
        "bot.DB.increment_notifications"
    ), patch(
        "bot.DB.update_user_count"
    ), patch(
        "bot.Scraper.scrape", return_value=[]
    ), patch(
        "bot.DB.increment_scrapes"
    ), patch(
        "bot.DB.get_all_grants", return_value=[]
    ), patch(
...
def test_title_truncation(fake_application):
    ...
    with patch(
        "bot.DB.get_pending_notifications", return_value=fake_notifications
    ), patch("bot.DB.mark_notification_sent", return_value=True), patch(
        "bot.DB.increment_notifications"
    ), patch(
        "bot.DB.update_user_count"
    ), patch(
...
def test_polling_configuration():
    ...
    with patch("bot.Application.builder", return_value=FakeApplicationBuilder()), patch(
        "bot.init_db"
    ), patch("bot.DB.get_or_create_stats"), patch("bot.DB.reset_started_at"), patch(
        "threading.Thread"
    ), patch(
        "signal.signal"
    ), patch(
        "bot.scrape_and_notify_loop", side_effect=fake_scrape_loop
    ):
