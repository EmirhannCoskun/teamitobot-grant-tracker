...
def test_status_command(fake_update, fake_context):
    ...
    with patch('bot.DB.get_stats_dict', return_value={
        'users': 10,
        'scrapes': 5,
        'started': None,
        'last_scrape': None
...
def test_unsubscribe_button(fake_update, fake_context):
    ...
    fake_update_obj = fake_update(chat_id=123, username="testuser", text="❌ Abone Olmaktan Çık")
...
def test_stats_button(fake_update, fake_context):
    ...
    fake_update_obj = fake_update(chat_id=123, username="testuser", text="📈 İstatistik")
    
    with patch('bot.DB.get_user_stats', return_value={
        'scrapes': 3,
        'notifications': 7,
        'subscribed': True
    }), patch('bot.DB.get_stats_dict', return_value={
        'users': 10,
        'scrapes': 5,
        'started': None,
        'last_scrape': None
...
def test_next_check_button(fake_update, fake_context):
    ...
    fake_update_obj = fake_update(chat_id=123, username="testuser", text="⏱️ Sonraki Tarama")
...
def test_notification_batching_5(fake_application):
    ...
    fake_notifications = [
        create_fake_notification(i, 123, f"Grant {i}", f"http://example.com/{i}", date(2024, 1, 1), date(2024, 12, 31))
        for i in range(1, 6)
    ]
    ...
    with patch('bot.DB.get_pending_notifications', return_value=fake_notifications), \
         ...
         patch('bot.config.CHECK_INTERVAL', 0):  # Force immediate scrape
...
def test_title_truncation(fake_application):
    ...
    long_title = "A" * 100
    fake_notifications = [
        create_fake_notification(1, 123, long_title, "http://example.com/1", date(2024, 1, 1), date(2024, 12, 31))
    ]
    ...
    with patch('bot.DB.get_pending_notifications', return_value=fake_notifications), \
        ...
        try:
            ...
            assert "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA..." in message
        finally:
            bot.last_scrape_time = original_time
...
def test_markdown_special_chars(fake_application):
    ...
    fake_notifications = [
        create_fake_notification(1, 123, special_title, "http://example.com/1", date(2024, 1, 1), date(2024, 12, 31))
    ]
...
def test_missing_optional_fields(fake_application):
    ...
    fake_notifications = [
        create_fake_notification(1, 123, "Grant 1", None, None, None)
    ]
...
def test_send_failure_behavior(fake_application):
    ...
    fake_notifications = [
        create_fake_notification(1, 123, "Grant 1", "http://example.com/1", date(2024, 1, 1), date(2024, 12, 31))
    ]
...
def test_polling_configuration():
    ...
    with patch('bot.Application.builder', return_value=FakeApplicationBuilder()), \
         patch('bot.init_db'), \
